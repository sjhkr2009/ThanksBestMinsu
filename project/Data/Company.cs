...
public class Company {
    ...
    public static Company CreateFromWeb(IWebDriver driver, int code) {
        ...
        List<string> marketCapString = SelectorHelper.GetValueByWeb(driver, SelectorHelper.Basic.MarketCap)
            .Replace(",", "").Replace(" ", "").Split('조').ToList();
        ret.MarketCap = marketCapString.Count > 1 ? marketCapString[0].ToInt32() * 10000 + marketCapString[1].ToInt32() : marketCapString[0].ToInt32();
        ...
        var headers = Enum.GetValues(typeof(SelectorHelper.Header));
